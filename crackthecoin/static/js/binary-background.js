...
function generateBinaryCode() {
  ...
  const binaryCodeLength = Math.floor((window.innerWidth/39)+15); // Adjust the length as needed
  let binaryCode = '';
...
function createBinaryBackground() {
  ...
  const lines = Math.floor((window.innerHeight/100)+15); // Adjust line count based on screen height
